    ...
    with app.app_context():  # <-- Add this context manager
        if request.method == "POST":
            ...
            if tool:
                ...
                cursor.execute(
                    "INSERT INTO Tools (name, company, url, thumbnail, "
                    "version, pricing) Values (%s, %s)",
                    (name, company, url, thumbnailUrl, version, pricing),
                )
                conn.commit()
                print(uploadMessage)
...
def review():
    with app.app_context():
        if request.method == "POST":
            ...
            cursor.execute(
                "INSERT INTO Review (idAccount, idIndex, review_text) VALUES (%s, %s, %s)",
...
def rating():
    with app.app_context():
        ...
        cursor.execute(
            "Insert into Rating (rating,idIndex,idAccount) VALUES (%s,%s,%s)",
